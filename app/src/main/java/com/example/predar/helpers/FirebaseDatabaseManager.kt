...
internal class FirebaseDatabaseManager(context: Context) {
    ...
    /** Listener for a new model uri from the Firebase Database.  */
    internal interface ModelUriListener {
        fun onModelUriAvailable(modelUri: String?)
    }
    ...
    fun storeUsingShortCode(shortCode: Int, cloudAnchorId: String) {
        rootRef.child(KEY_ANCHOR + shortCode).setValue(cloudAnchorId)
    }

    /** Stores the model uri configured Firebase Database.  */
    fun storeModelUri(modelUri: String) {
        rootRef.child(KEY_MODEL_URI).setValue(modelUri)
    }

    /**
     * Retrieves a model uri to be displayed. Returns an empty string if a model uri was not stored.
     */
    fun getModelUri(listener: ModelUriListener) {
        rootRef
            .child(KEY_MODEL_URI)
            .addValueEventListener(
                object : ValueEventListener {
                    override fun onDataChange(dataSnapshot: DataSnapshot) {
                        listener.onModelUriAvailable(dataSnapshot.value.toString())
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Log.e(
                            TAG, "The database operation for getModelUri was cancelled.",
                            error.toException()
                        )
                        listener.onModelUriAvailable(null)
                    }
                })
    }
    ...
    companion object {
        ...
        private val KEY_ANCHOR = "anchor;"
        private val KEY_MODEL_URI = "model_uri"
